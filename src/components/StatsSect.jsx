...
function StatsSect({theme, icon, text, data, rise, fall, up, down, trend}) {
    ...
    // }, [])
  return (
    <div className={`w-[48%] flex flex-col items-start justify-between bg-red-40 xs:max-lg:w-[49%] md:max-lg:w-[45%] md:max-lg:mx-auto xs:max-lg:space-y-3 xs:max-md:space-y-1 xs:max-lg:p-3 xs:max-lg:h-auto xs:max-lg:my-2 border border-[#EDF2F7] ${theme === "light" ? "bg-white" : "bg-gray-800" } rounded-xl p-2 border border-[#EDF2F7]`}>
    ...
    <p className={`font-jarkarta font-medium xs:max-md:text-gray-900 text-base my-2  ${theme === "light" ? "text-[#898989]" : "text-slate-300" } `}>
      {text}
    </p>
    <p onClick={() => handleToggleInfo() } className={`font-jarkarta font-semibold text-2xl xs:max-md:text-lg ${theme === "light" ? "text-[#3A3F51]" : "text-slate-200" } `}>
      {toggleInfo ? data.diff : text === "Total Income" ? `$${data.today}` : data.today }
    </p>
    <div className="flex xs:max-md:flex-col gap-2 bg-red-40">
      <div className={`flex xs:max-md:w-auto items-center justify-center gap-2 p-1 rounded-md ${data.diff === 0 ? "bg-blue-400" : data.diff > 0 ? "bg-[#34CAA5]" : "bg-red-400" } bg-opacity-15`}>
      ...
      <p className={`font-inter font-normal text-sm xs:max-md:text-xs xs:max-md:text-right xs:max-md:text-gray-800 ${theme === "light" ? "text-[#606060]" : "text-slate-400" } `}>{`vs previous ${trend}`}</p>
    </div>
  </div>
  )
}
