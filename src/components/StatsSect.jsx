...
function StatsSect({theme, icon, text, data, rise, fall, up, down, trend}) {
    ...
    // }, [])
  return (
    <div className={`w-[48%] flex flex-col items-start justify-between bg-red-40 xs:max-lg:w-[49%] md:max-lg:w-[45%] md:max-lg:mx-auto xs:max-lg:space-y-3 xs:max-md:space-y-1 xs:max-lg:p-3 xs:max-lg:h-auto xs:max-lg:my-2 border border-[#EDF2F7] ${theme === "light" ? "bg-white hover:bg-slate-100" : "bg-gray-800 hover:bg-gray-700" } rounded-xl p-2 border border-[#EDF2F7]`}>
    ...
    <p onClick={() => handleToggleInfo() } className={`cursor-pointer w-full font-jarkarta font-semibold text-2xl xs:max-md:text-lg ${theme === "light" ? "text-[#3A3F51]" : "text-slate-200" } `}>
    ...
    <div className="flex xs:max-md:flex-col xs:max-md:w-full gap-2 bg-red-40">
      ...
      <p className={`font-inter font-normal text-sm xs:max-md:text-xs xs:max-md:text-right xs:max-md:text-gray-300 ${theme === "light" ? "text-[#606060]" : "text-slate-400" } `}>{`vs previous ${trend}`}</p>
    </div>
  </div>
  )
}
